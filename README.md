...
### Main User Flow
**1. Login / Register**
Users create an account or log in at `/login` or `/register`. After successful authentication they land on the Dashboard which shows their personal stats and navigation options.
**2. Lobby Creation & Joining**
From the Dashboard players can create a new lobby or join an existing one. Private lobbies are joined via session code, public lobbies via the open lobby list at `/lobby/join`. The host can invite players, configure game timers and set move visibility before starting.
...
> ![Game Interface](./public/screenshots/game.png)
